...
int MEMPHY_write(struct memphy_struct *mp, int addr, BYTE data)
{
   if (mp == NULL)
   {  
      // printf("memphy write failed \n");
      return -1;
   }
   if (mp->rdmflg)
   {
      // printf("memphy rdmfield write \n");
      mp->storage[addr] = data;
   }
   else /* Sequential access device */
   {
   //    printf("memphy seq write \n");
...
int MEMPHY_dump(struct memphy_struct *mp)
{
   ...
   int i = 0;
   // printf("max memphy size: %d\n", mp->maxsz);
   while (i < mp->maxsz)
   {
      // if(mp->storage[i] + mp->storage[i + 1] + mp->storage[i + 2] + mp->storage[i + 3]){
      //    printf("%04x: ", i);
